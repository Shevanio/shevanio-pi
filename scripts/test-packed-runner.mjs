...
try {
	...
	Object.assign(process.env, { HOME: runtimeHome, USERPROFILE: runtimeHome, SHEVANIO_PI_CONFIG_HOME: join(runtimeHome, "shevanio-config"), GENTLE_PI_CONFIG_HOME: join(runtimeHome, "config"), GENTLE_PI_AGENT_HOME: runtimeAgentDir, PI_OFFLINE: "1" });
	delete process.env.GENTLE_PI_NO_SKILL_REGISTRY;
	delete process.env.GENTLE_PI_AUTONOMOUS_MODE;
	const gitignorePath = join(runtimeCwd, ".gitignore"), legacyRegistryPath = join(runtimeCwd, ".pi", "extensions", "skill-registry.ts"), packedSkillPath = join(runtimeCwd, "skills", "packed-proof", "SKILL.md");
	const gitignoreBytes = Buffer.from("# packed sentinel\r\ndist/\r\n"), legacyRegistryBytes = Buffer.from(`export default function customProjectSkillRegistry(_pi: unknown) {}
...
// Packed project customization: preserve these exact bytes.
`);
	mkdirSync(dirname(legacyRegistryPath), { recursive: true }); mkdirSync(dirname(packedSkillPath), { recursive: true });
	writeFileSync(gitignorePath, gitignoreBytes); writeFileSync(legacyRegistryPath, legacyRegistryBytes); writeFileSync(packedSkillPath, "---\nname: packed-proof\ndescription: Packed proof.\n---\n");
	...
	if (!notices.some(({ message, type }) => type === "warning" && message === `Legacy skill registry extension detected at ${legacyRegistryPath}. Resolve it manually: remove or relocate that file to use only the packaged registry. Shevanio Pi will not inspect or modify it.`)) throw new Error("packed startup did not report the project-owned legacy registry path");
	if (!readFileSync(gitignorePath).equals(gitignoreBytes) || !readFileSync(legacyRegistryPath).equals(legacyRegistryBytes)) throw new Error("packed startup changed project-owned registry sentinels");
	if (readdirSync(dirname(legacyRegistryPath)).join(",") !== "skill-registry.ts") throw new Error("packed startup created a disabled legacy registry derivative");
	if (!readFileSync(join(runtimeCwd, ".atl", "skill-registry.md"), "utf8").includes("packed-proof") || typeof JSON.parse(readFileSync(join(runtimeCwd, ".atl", ".skill-registry.cache.json"), "utf8")).fingerprint !== "string") throw new Error("packed startup did not generate managed registry outputs");
	await session.prompt("/skill-registry:refresh");
	if (!readFileSync(gitignorePath).equals(gitignoreBytes) || !readFileSync(legacyRegistryPath).equals(legacyRegistryBytes)) throw new Error("packed refresh changed project-owned registry sentinels");
	if (readdirSync(join(runtimeCwd, ".atl")).sort().join(",") !== ".skill-registry.cache.json,skill-registry.md") throw new Error("packed refresh wrote outside the managed registry outputs");
