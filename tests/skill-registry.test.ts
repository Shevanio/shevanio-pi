...
import registerSkillRegistry, { __testing } from "../extensions/skill-registry.ts";
...
test("registry renders indexed skill paths instead of compact rules", () => {
	...
	assert.match(registry, /<!-- Auto-generated by shevanio-pi extensions\/skill-registry\.ts\. Run \/skill-registry:refresh to regenerate\. -->/);
	assert.doesNotMatch(registry, /Auto-generated by gentle-pi/);
...
test("schema version 8 invalidates a schema 7 registry fingerprint", async () => {
	const cwd = join(tmpdir(), `gentle-pi-schema-fingerprint-${Date.now()}`);
	const skillPath = join(cwd, "skills", "alpha", "SKILL.md");
	mkdirSync(dirname(skillPath), { recursive: true });
	writeFileSync(skillPath, "---\nname: alpha\ndescription: Alpha.\n---\n");
	const dirs = await __testing.uniqueExistingDirs([
		...__testing.projectSkillDirs(cwd),
		...__testing.userSkillDirs(),
	]);
	const files: string[] = [];
	for (const dir of dirs) files.push(...(await __testing.findSkillFiles(dir)));
	const staleFingerprint = await __testing.fingerprint(files, 7);
	mkdirSync(join(cwd, ".atl"), { recursive: true });
	writeFileSync(join(cwd, ".atl", "skill-registry.md"), "stale schema 7 registry\n");
	writeFileSync(join(cwd, ".atl", ".skill-registry.cache.json"), JSON.stringify({ fingerprint: staleFingerprint }));

	assert.equal(__testing.registrySchemaVersion, 8);
	const regenerated = await __testing.regenerateRegistry(cwd, false);
	assert.deepEqual(regenerated.reason, "fingerprint-changed");
	assert.match(readFileSync(join(cwd, ".atl", "skill-registry.md"), "utf8"), /Auto-generated by shevanio-pi/);
	assert.equal((await __testing.regenerateRegistry(cwd, false)).reason, "cache-hit");
});

test("registered startup and refresh preserve project-owned files and write only managed registry outputs", async () => {
	const cwd = join(tmpdir(), `gentle-pi-owned-files-${Date.now()}`);
	const gitignorePath = join(cwd, ".gitignore");
	const legacyPath = join(cwd, ".pi", "extensions", "skill-registry.ts");
	const skillPath = join(cwd, "skills", "alpha", "SKILL.md");
	const gitignoreBytes = Buffer.from("# project sentinel\r\nbuild/\r\n");
	const legacyBytes = Buffer.from(`// Auto-generated by .pi/extensions/skill-registry.ts
...
// Project modification: preserve these exact bytes.
`);
	...
	const warning = `Legacy skill registry extension detected at ${legacyPath}. Resolve it manually: remove or relocate that file to use only the packaged registry. Shevanio Pi will not inspect or modify it.`;
	assert.ok(notifications.some((entry) => entry.level === "warning" && entry.message === warning));
	assert.deepEqual(readFileSync(gitignorePath), gitignoreBytes);
	assert.deepEqual(readFileSync(legacyPath), legacyBytes);
	assert.deepEqual(readdirSync(dirname(legacyPath)), ["skill-registry.ts"]);
	assert.match(readFileSync(join(cwd, ".atl", "skill-registry.md"), "utf8"), /\| `alpha` \|/);
	assert.deepEqual(Object.keys(JSON.parse(readFileSync(join(cwd, ".atl", ".skill-registry.cache.json"), "utf8"))), ["fingerprint"]);

	await commands.get("skill-registry:refresh")!.handler("", ctx);
	assert.deepEqual(readFileSync(gitignorePath), gitignoreBytes);
	assert.deepEqual(readFileSync(legacyPath), legacyBytes);
	assert.deepEqual(readdirSync(dirname(legacyPath)), ["skill-registry.ts"]);
	assert.deepEqual(readdirSync(join(cwd, ".atl")).sort(), [".skill-registry.cache.json", "skill-registry.md"]);
