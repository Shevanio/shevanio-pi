...
async function run() {
	...
	const ownershipRegistryCwd = await tempWorkspace();
	const ownershipRegistryAgentDir = await tempWorkspace();
	const previousRegistrySkip = process.env.GENTLE_PI_NO_SKILL_REGISTRY;
	try {
		const gitignorePath = join(ownershipRegistryCwd, ".gitignore");
		const legacyPath = join(ownershipRegistryCwd, ".pi", "extensions", "skill-registry.ts");
		const skillPath = join(ownershipRegistryCwd, "skills", "runtime-proof", "SKILL.md");
		const gitignoreBytes = Buffer.from("# runtime sentinel\r\nnode_modules/\r\n");
		const legacyBytes = Buffer.from(`export default function customProjectSkillRegistry(_pi: unknown) {}
// Auto-generated by .pi/extensions/skill-registry.ts
const REGISTRY_REL_PATH = ".atl/skill-registry.md";
function projectSkillDirs(cwd: string): string[] { return []; }
function regenerateRegistry(cwd: string, force: boolean) {}
// Runtime project customization: preserve these exact bytes.
`);
		await mkdir(dirname(legacyPath), { recursive: true });
		await mkdir(dirname(skillPath), { recursive: true });
		await writeFile(gitignorePath, gitignoreBytes);
		await writeFile(legacyPath, legacyBytes);
		await writeFile(skillPath, "---\nname: runtime-proof\ndescription: Runtime proof.\n---\n");
		delete process.env.GENTLE_PI_NO_SKILL_REGISTRY;
		const registryLoader = new DefaultResourceLoader({ cwd: ownershipRegistryCwd, agentDir: ownershipRegistryAgentDir, additionalExtensionPaths: [join(ROOT, "extensions", "skill-registry.ts")], noSkills: true, noPromptTemplates: true, noThemes: true, noContextFiles: true });
		await registryLoader.reload();
		assert.deepEqual(registryLoader.getExtensions().errors, []);
		const { session: registrySession } = await createAgentSession({ cwd: ownershipRegistryCwd, agentDir: ownershipRegistryAgentDir, resourceLoader: registryLoader, sessionManager: SessionManager.inMemory(ownershipRegistryCwd), noTools: "builtin" });
		const registryRunner = registrySession.extensionRunner;
		const notices = [];
		registryRunner.setUIContext({ ...registryRunner.getUIContext(), notify(message, type = "info") { notices.push({ message, type }); } });
		await registryRunner.emit({ type: "session_start" });
		assert.ok(notices.some(({ message, type }) => type === "warning" && message === `Legacy skill registry extension detected at ${legacyPath}. Resolve it manually: remove or relocate that file to use only the packaged registry. Shevanio Pi will not inspect or modify it.`));
		assert.deepEqual(await readFile(gitignorePath), gitignoreBytes);
		assert.deepEqual(await readFile(legacyPath), legacyBytes);
		assert.deepEqual(await readdir(dirname(legacyPath)), ["skill-registry.ts"]);
		assert.match(await readFile(join(ownershipRegistryCwd, ".atl", "skill-registry.md"), "utf8"), /runtime-proof/);
		await registrySession.prompt("/skill-registry:refresh");
		assert.deepEqual(await readFile(gitignorePath), gitignoreBytes);
		assert.deepEqual(await readFile(legacyPath), legacyBytes);
		assert.deepEqual((await readdir(join(ownershipRegistryCwd, ".atl"))).sort(), [".skill-registry.cache.json", "skill-registry.md"]);
		await registryRunner.emit({ type: "session_shutdown", reason: "quit" });
	} finally {
		if (previousRegistrySkip === undefined) delete process.env.GENTLE_PI_NO_SKILL_REGISTRY;
		else process.env.GENTLE_PI_NO_SKILL_REGISTRY = previousRegistrySkip;
		await rm(ownershipRegistryCwd, { recursive: true, force: true });
		await rm(ownershipRegistryAgentDir, { recursive: true, force: true });
	}
