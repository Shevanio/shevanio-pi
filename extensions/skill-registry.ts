...
async function fingerprint(files: string[], schemaVersion = REGISTRY_SCHEMA_VERSION): Promise<string> {
	const lines: string[] = [`schema:${schemaVersion}`];
...
function renderRegistry(cwd: string, sources: string[], entries: SkillEntry[]): string {
	...
	lines.push("");
	lines.push("<!-- Auto-generated by shevanio-pi extensions/skill-registry.ts. Run /skill-registry:refresh to regenerate. -->");
	lines.push("");
...
export default function (pi: ExtensionAPI) {
	...
	pi.registerFlag(NO_SKILL_REGISTRY_FLAG, {
		description: "Skip the Shevanio Pi skill registry refresh and watcher on startup.",
		type: "boolean",
		default: false,
	});
	...
	pi.on("session_start", async (_event, ctx) => {
		...
		try {
			const legacyPath = join(ctx.cwd, LEGACY_PROJECT_REGISTRY_REL_PATH);
			if (ctx.hasUI && (await pathExists(legacyPath))) {
				ctx.ui.notify(
					`Legacy skill registry extension detected at ${legacyPath}. Resolve it manually: remove or relocate that file to use only the packaged registry. Shevanio Pi will not inspect or modify it.`,
					"warning",
				);
			}
